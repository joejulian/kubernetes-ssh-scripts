...
gen_etcd_client_cert() {
    ...
    ( cfssl gencert -ca=${CA_DIR}/ca.pem -ca-key=${CA_DIR}/ca-key.pem -config=${CA_CONFIG_DIR}/ca-config.json -profile=client ${ETCD_CLIENT_CONFIG} | cfssljson -bare client ) 2> /dev/null
